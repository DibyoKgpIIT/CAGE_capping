...
df =  pd.read_csv("octamer_sequence_df_10_dist.csv")
positive_motifs = {}
negative_motifs = {}
...
for ind in range(len(df.index)):
	seq = df["seq"].loc[ind]
	octamer_list_str = df["octamer"].loc[ind]
	predicted_class = df["class"].loc[ind]
	octamer_list = octamer_list_str.replace("\"","")
	octamer_list = octamer_list.replace("[","")
	octamer_list = octamer_list.replace("]","")
	octamer_list = octamer_list.split(",")
	octamer_list = [octamer.strip(" ") for octamer in octamer_list]
	octamer_list = [octamer.strip("\'") for octamer in octamer_list]
	print(octamer_list)
"""
...
p_value_dict = {}
for permutation in fifty_percent_delta_motifs:
...
all_keys = sorted(p_value_dict, key=p_value_dict.get)
motifs = []
p_values = []
num_pos = []
num_neg = [] 
for key in all_keys:
	if p_value_dict[key]<0.01:
		print("motif:",key)
...
motif_df.to_csv("motif_df.csv",index=False)
motif_seqs=[]
motifs = []
p_values = []
num_poses = []
num_negs = []
for i in range(len(motif_df.index)):
	motif = motif_df["motif"].loc[i]
	p_value = motif_df["p_value"].loc[i]
	num_pos = motif_df["num_pos"].loc[i]
	num_neg = motif_df["num_neg"].loc[i]
	cnt = 0
	motif_txtfile = ""
	for ind in range(len(df.index)):
		seq = df["seq"].loc[ind]
		...
		octamer_list = octamer_list.split(",")
		octamer_list = [octamer.strip(" ") for octamer in octamer_list]
		if (("\'"+motif+"\'" in octamer_list)==True) and ((motif in motifs and seq in motif_seqs)==False):
			motif_seq = seq
			...
			cnt+=1
			motif_txtfile += ">"+motif+"."+str(cnt)+" "+str(p_value)+" "+str(num_pos)+" "+str(num_neg)+"\n"+seq+"\n"
	with open("motifs_with_p_vals_and_seqs_10_dist_"+str(motif)+".fa","w") as file:
        	file.write(motif_txtfile)
motif_df2 = pd.DataFrame(data=list(zip(motifs,p_values,num_poses,num_negs,motif_seqs)),columns=["motif","p_value","num_pos","num_neg","seq"])
motif_df2.to_csv("motif_df2_10_dist.csv",index=False)
with open("motifs_with_p_vals_and_seqs_10_dist.fa","w") as file:
	file.write(motif_txtfile)
fig = plt.subplots(figsize =(40, 8))
x = np.arange(len(motif_df.index))  # the label locations
width = 0.25  # the width of the bars
multiplier = 0
#plt.figure(1, figsize=(50,10))
...
plt.xlabel('Motifs') 
plt.ylabel('Frequency')
plt.xticks(x,motif_df["motif"].to_list())
#plt.twinx()
plt.legend()
plt.savefig("lowest_p_val_motifs_10_dist.png")
