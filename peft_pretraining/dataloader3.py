...
def tokenize_and_chunk(
    ...
    sequence_list = ["".join(train_df[text_field].loc[0][i:i+8]) for i in range(0,sequence_length-8,4)]
                        #["".join(train_df[text_field].loc[0][i+2:i+10]) for i in range(0,sequence_length-10,4)]
                        #["".join(train_df[text_field].loc[0][i+1:i+9]) for i in range(0,sequence_length-9,4)]+
                        #["".join(train_df[text_field].loc[0][i+3:i+11]) for i in range(0,sequence_length-11,4)]
    ...
    tokenized_dataset = dataset.map(
        lambda example: tokenizer(" ".join(["".join(example[text_field][i:i+8]) for i in range(0,sequence_length-8,4)]).strip(" ")+tokenizer.eos_token),
    ...
    def group_texts(examples):
        ...
        concatenated_examples["label"] = examples["label"]
