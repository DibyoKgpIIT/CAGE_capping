...
from peft_pretraining.dataloader2 import SkipDataLoader
...
def main(args):
    ...
    if args.dataset_path is not None:
        ...
        train_dataset2 = train_dataset.to_pandas()
        eval_dataset2 = eval_dataset.to_pandas()
        """
        train_input_ids = []
        eval_input_ids = []
        for ind in range(len(train_dataset.index)):
            input_ids = train_dataset["input_ids"].loc[ind]
            max_len = max(len(arr) for arr in input_ids)
            padded_input_ids = [np.pad(arr, (0, max_len - len(arr)), mode='constant') for arr in input_ids]
            train_input_ids.append(padded_input_ids)
        for ind in range(len(eval_dataset.index)):
            input_ids = eval_dataset["input_ids"].loc[ind]
            max_len = max(len(arr) for arr in input_ids)
            padded_input_ids = [np.pad(arr, (0, max_len - len(arr)), mode='constant') for arr in input_ids]
            eval_input_ids.append(padded_input_ids)
        """
        train_dataset2 = pd.DataFrame(data=list(zip(train_dataset2.index,train_dataset2["input_ids"].to_list())),columns=["index","input_ids"])
        eval_dataset2 = pd.DataFrame(data=list(zip(eval_dataset2.index,eval_dataset2["input_ids"].to_list())),columns=["index","input_ids"])
        print("train_dataset",train_dataset2)
    ...
    #batch0 = next(batch_iterator)
    #print("k[0]:",batch0.items()[0],",v[0]:",batch0.items()[1])
