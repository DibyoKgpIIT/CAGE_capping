...
def main():
    ...
    if data_args.task_name is not None:
        ...
        training_args.num_training_steps = 37500
    ...
    if data_args.task_name is not None:
        ...
        is_regression = raw_datasets["train"]["label"].dtype in ["float32", "float64"]
        if is_regression:
            num_labels = 1
        else:
            # A useful fast method:
            # https://huggingface.co/docs/datasets/package_reference/main_classes.html#datasets.Dataset.unique
            label_list = raw_datasets["train"]["label"].unique()
    ...
    if data_args.task_name is not None:
        ...
        non_label_column_names = [name for name in raw_datasets["train"].columns if name != "label"]
    ...
    if training_args.do_train:
        ...
        if data_args.max_train_samples is not None:
            ...
            train_dataset = train_dataset.sample(n=max_train_samples,ignore_index=True)
    ...
    if training_args.do_eval:
        ...
        if data_args.max_eval_samples is not None:
            ...
            eval_dataset = eval_dataset.sample(n=max_eval_samples,ignore_index=True)
    ...
    if training_args.do_predict or data_args.task_name is not None or data_args.test_file is not None:
        ...
        if data_args.max_predict_samples is not None:
            ...
            predict_dataset = predict_dataset.sample(n=max_predict_samples,ignore_index=True)

    # Log a few random samples from the training set:
    if training_args.do_train:
        for index in random.sample(range(len(train_dataset)), 3):
            logger.info(f"Sample {index} of the training set: {train_dataset.loc[index]}.")
    ...
    class DataFrameDataset(Dataset):
        ...
        def __getitem__(self, idx):
            values=np.hstack(self.df.iloc[idx].values).astype(np.float64)
            return torch.tensor(values)
    train_dataset = DataFrameDataset(train_dataset)
    eval_dataset = DataFrameDataset(eval_dataset)
    
    #train_dataloader = DataLoader(dataset, batch_size=24)
