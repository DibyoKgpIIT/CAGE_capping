...
class LayerCaptureCallback(TrainerCallback):
    ...
    def on_prediction_step(self, args=TrainingArguments, state=TrainerState,
                           control=TrainerControl, inputs=None, outputs=None, model=None, **kwargs):
...
def main():
    ...
    if data_args.task_name is not None:
        ...
        minimum_n_tokens = training_args.total_batch_size * \
    ...
    hook = model.model.layers[3].post_attention_layernorm.register_forward_hook(
        hook_fn)
    ...
    if (
        ...
        if sorted(label_name_to_id.keys()) == sorted(label_list):
            ...
            logger.warning(
                ...
                f"model labels: {
                    sorted(
                        label_name_to_id.keys())}, dataset labels: {
                    sorted(label_list)}."
    ...
    if data_args.max_seq_length > tokenizer.model_max_length:
        logger.warning(
            f"The max_seq_length passed ({
                data_args.max_seq_length}) is larger than the maximum length for the"
            f"model ({
    ...
    # predictions and label_ids field) and has to return a dictionary string
    # to float.
    ...
    if training_args.do_predict:
        ...
        quadmer_permutations = deBAG.init_all_kmer_permutation_counts(
            'ATCG', 4)
        pentamer_permutations = deBAG.init_all_kmer_permutation_counts(
            'ATCG', 5)
        hexamer_permutations = deBAG.init_all_kmer_permutation_counts(
            'ATCG', 6)
        heptamer_permutations = deBAG.init_all_kmer_permutation_counts(
            'ATCG', 7)
        octamer_permutations = deBAG.init_all_kmer_permutation_counts(
            'ATCG', 8)
        ...
        for predict_dataset, task in zip(predict_datasets, tasks):
            ...
            for i in range(len(predict_dataset)):
                ...
                normalized_attention_map = avg_attention_map - \
                    np.min(avg_attention_map) / \
                    (np.max(avg_attention_map) - np.min(avg_attention_map))
                avg_attention_values = np.zeros(
                    (avg_attention_map.shape[0]), dtype=float)
                for row in range(avg_attention_map.shape[0]):
                    avg_attention_values[row] = np.sum(
                        normalized_attention_map[row, :])
                ...
                decoded_token_full_seq = tokenizer.decode(
                    predict_dataset[0]["input_ids"])
                decoded_token_full_seq_no_space = decoded_token_full_seq.replace(
                    " ", "")
                decoded_token_full_seq_no_space = decoded_token_full_seq_no_space.replace(
                    "<|endoftext|>", "")
                decoded_token_full_seq_no_space_trimers = deBAG.get_kmer_count_from_sequence(
                    decoded_token_full_seq_no_space, k=3, cyclic=False)
                decoded_token_full_seq_no_space_quadmers = deBAG.get_kmer_count_from_sequence(
                    decoded_token_full_seq_no_space, k=4, cyclic=False)
                decoded_token_full_seq_no_space_pentamers = deBAG.get_kmer_count_from_sequence(
                    decoded_token_full_seq_no_space, k=5, cyclic=False)
                decoded_token_full_seq_no_space_hexamers = deBAG.get_kmer_count_from_sequence(
                    decoded_token_full_seq_no_space, k=6, cyclic=False)
                decoded_token_full_seq_no_space_heptamers = deBAG.get_kmer_count_from_sequence(
                    decoded_token_full_seq_no_space, k=7, cyclic=False)
                decoded_token_full_seq_no_space_octamers = deBAG.get_kmer_count_from_sequence(
                    decoded_token_full_seq_no_space, k=8, cyclic=False)
                if predicted_class_name == "LABEL_0":
                ...
                for k in range(3):
                    decoded_token = tokenizer.decode(predict_dataset[0]["input_ids"]
                                                     [attention_values_sorted[-(k + 1)] - 10:attention_values_sorted[-(k + 1)] + 10])
                    decoded_token_no_space = decoded_token.replace(" ", "")
                    decoded_token_no_space = decoded_token_no_space.replace(
                        "<|endoftext|>", "")
                    sequence_fragments.append(decoded_token_no_space)
                    decoded_token_no_space_trimers = deBAG.get_kmer_count_from_sequence(
                        decoded_token_no_space, k=3, cyclic=False)
                    decoded_token_no_space_quadmers = deBAG.get_kmer_count_from_sequence(
                        decoded_token_no_space, k=4, cyclic=False)
                    decoded_token_no_space_pentamers = deBAG.get_kmer_count_from_sequence(
                        decoded_token_no_space, k=5, cyclic=False)
                    decoded_token_no_space_hexamers = deBAG.get_kmer_count_from_sequence(
                        decoded_token_no_space, k=6, cyclic=False)
                    decoded_token_no_space_heptamers = deBAG.get_kmer_count_from_sequence(
                        decoded_token_no_space, k=7, cyclic=False)
                    decoded_token_no_space_octamers = deBAG.get_kmer_count_from_sequence(
                        decoded_token_no_space, k=8, cyclic=False)
                    octamers.append(
                        list(decoded_token_no_space_octamers.keys()))
                    ...
                    upe_match = upe_regex.search(
                        decoded_token.replace(" ", ""))
                    dpe_match = dpe_regex.search(
                        decoded_token.replace(" ", ""))
                    tata_match = tata_regex.search(
                        decoded_token.replace(" ", ""))
                    inr_match = inr_regex.search(
                        decoded_token.replace(" ", ""))
                    if predicted_class_name == "LABEL_0":
            ...
            octamer_sequence_df = pd.DataFrame(data=list(
                zip(sequence_fragments, octamers, classes)), columns=["seq", "octamer", "class"])
            octamer_sequence_df.to_csv(
                "octamer_sequence_df_10_dist.csv", index=False)
            hook.remove()
            hook2.remove()
            ...
            for k in range(3):
                print("upe_matches_predicted0_" + str(k + 1),
                      upe_matches_predicted0[k] / num_predicted_negative)
                print("dpe_matches_predicted0_" + str(k + 1),
                      dpe_matches_predicted0[k] / num_predicted_negative)
                print("inr_matches_predicted0_" + str(k + 1),
                      inr_matches_predicted0[k] / num_predicted_negative)
                print("tata_matches_predicted0_" + str(k + 1),
                      tata_matches_predicted0[k] / num_predicted_negative)
                print("upe_matches_predicted1_" + str(k + 1),
                      upe_matches_predicted1[k] / num_predicted_positive)
                print("dpe_matches_predicted1_" + str(k + 1),
                      dpe_matches_predicted1[k] / num_predicted_positive)
                print("inr_matches_predicted1_" + str(k + 1),
                      inr_matches_predicted1[k] / num_predicted_positive)
                print("tata_matches_predicted1_" + str(k + 1),
                      tata_matches_predicted1[k] / num_predicted_positive)
            ...
            print(
                "upe_matches_predicted0_union",
                upe_matches_predicted0_union /
                num_predicted_negative)
            print(
                "dpe_matches_predicted0_union",
                dpe_matches_predicted0_union /
                num_predicted_negative)
            print(
                "inr_matches_predicted0_union",
                inr_matches_predicted0_union /
                num_predicted_negative)
            print(
                "tata_matches_predicted0_union",
                tata_matches_predicted0_union /
                num_predicted_negative)
            print(
                "upe_matches_predicted1_union",
                upe_matches_predicted1_union /
                num_predicted_positive)
            print(
                "dpe_matches_predicted1_union",
                dpe_matches_predicted1_union /
                num_predicted_positive)
            print(
                "inr_matches_predicted1_union",
                inr_matches_predicted1_union /
                num_predicted_positive)
            print(
                "tata_matches_predicted1_union",
                tata_matches_predicted1_union /
                num_predicted_positive)
            ...
            for key in predicted_positive_vocab_full_seq.keys():
                if predicted_positive_vocab_full_seq[key] > 0:
                    print(
                        str(key) + ":",
                        predicted_positive_vocab_full_seq[key])
            ...
            for key in predicted_negative_vocab_full_seq.keys():
                if predicted_negative_vocab_full_seq[key] > 0:
                    print(
                        str(key) + ":",
                        predicted_negative_vocab_full_seq[key])
